...
// Dipole-Dipole configuration for 8 electrodes
// Format: {current+, voltage+, voltage-, current-}
const int dipoleDipoleConfig[][4] = {
...
const float CURRENT_LIMIT_RESISTOR = 2000.0; // 2k ohm current-limiting resistor
const float ESP32_VOLTAGE = 3.3;             // ESP32 supply voltage
const float ACTUAL_CURRENT = ESP32_VOLTAGE / CURRENT_LIMIT_RESISTOR;

float measurements[numConfigurations];  // Store impedance values

void setup() {
  Serial.begin(115200);
  for (int i = 0; i < 8; i++) {
    pinMode(electrodePins[i], INPUT); // Initialize all electrodes as high impedance
  }
}
...
void loop() {
  if (Serial.available()) {
    ...
    runMeasurementCycles();
  }
}

void runMeasurementCycles() {
...
void performMeasurements() {
  for (int i = 0; i < numConfigurations; i++) {
    int currentPos = dipoleDipoleConfig[i][0];
    int voltagePos = dipoleDipoleConfig[i][1];
    int voltageNeg = dipoleDipoleConfig[i][2];
    int currentNeg = dipoleDipoleConfig[i][3];

    setCurrent(currentPos, currentNeg);
    delay(10);  // Allow current to stabilize

    float voltage = measureVoltage(voltagePos, voltageNeg);
...
void setCurrent(int electrode1, int electrode2) {
  // Reset all electrodes to high impedance
  for (int i = 0; i < 8; i++) {
    pinMode(electrodePins[i], INPUT);
  }

  // Inject current between two electrodes
